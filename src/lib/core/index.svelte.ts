import type { I18nConfig, NestedTranslations, Paths, PluralObject, TranslationValue } from './types';
...
export const setupI18n = <Locale extends string, BaseLanguage extends Record<string, unknown>>(
	...
	type Translations = Record<Locale, Paths<BaseLanguage>>;
	...
	const translations = $state<Translations>({
		[config.fallbackLocale]: fallbackLocalization
	...
	function getSavedLocale(): Locale {
		...
		return config.fallbackLocale as Locale;
	}

	// Browser language detection helper
	function detectBrowserLocale(): Locale {
		if (typeof navigator === 'undefined') {
			return config.fallbackLocale as Locale; // Return fallback if navigator is not available (SSR)
		}
		const browserLang = navigator.language.split('-')[0];
		return isValidLocale(browserLang) ? browserLang : (config.fallbackLocale as Locale);
	}

	// Get a nested value from an object using dot notation safely
	function getNestedValue(
		obj: NestedTranslations | undefined,
		path: string
	...
	function findTranslationValue(
		...
		if (value === undefined && primaryLocale !== fallbackLocale) {
			...
			const fallbackObj = allTranslations[fallbackLocale] as unknown as
				| NestedTranslations
				| undefined;
			value = getNestedValue(fallbackObj, key);
			if (config.debug && value !== undefined) {
				console.warn(
					`Translation key '${key}' not found in '${primaryLocale}', using fallback '${fallbackLocale}'.`
				);
			}
		}
		return value;
		...
		return str.replace(/{([^{}]+)}/g, (match, expr: string) => {
			...
			if (Object.prototype.hasOwnProperty.call(params, key)) {
				...
				const isDateLike =
					...
					(typeof value === 'string' && !isNaN(Date.parse(String(value)))) ||
					typeof value === 'number';
				if (formatType && isDateLike) {
					try {
						...
						switch (formatType) {
							...
							case 'relative': {
								// TODO: Implement relative time formatting with type safety
							...
							default:
								...
								if (
									...
									formatType.charAt(0) === '{' &&
									formatType.charAt(formatType.length - 1) === '}'
								) {
									try {
						...
						if (config.debug)
							console.warn(`Error formatting date value for key "${key}":`, value, e);
				...
				if (typeof value === 'number') {
					try {
								...
								default: {
									...
									if (!isNaN(precision) && precision >= 0) {
										...
										try {
											...
											if (config.debug)
												console.warn(`Invalid number format options: ${formatType}`, e);
						...
						if (config.debug)
							console.warn(`Error formatting number value for key "${key}":`, value, e);
						return String(value); // Return original value on formatting error
					}
				}
				...
				if (Array.isArray(value)) {
					...
					if (formatType === 'list' && typeof Intl !== 'undefined' && 'ListFormat' in Intl) {
						try {
							...
							if (config.debug)
								console.warn(`Error using Intl.ListFormat for key "${key}":`, value, e);
	 ...
	 */
	function initLocale(options: InitLocaleOptions = {}): void {
		const opts: Required<InitLocaleOptions> = {
			preferBrowser: true,
			preferStorage: true,
			defaultLocale: config.fallbackLocale as Locale,
			...options
		};
		...
		if (config.debug) {
			console.log(
				`i18n initialized. Locale set to: ${currentLocale}. Strategy: ${detectedLocale ? (opts.preferStorage && localStorage.getItem(config.localStorageKey) ? 'Storage' : 'Browser') : 'Default/Fallback'}`
			);
		}
	}
	 ...
	 */
	async function setLocale(newLocale: Locale, options: SetLocaleOptions = {}): Promise<void> {
		...
		if (failedLocaleLoads[newLocale]) {
			...
			if (currentLocale === newLocale) {
				currentLocale = config.fallbackLocale as Locale;
			}
			return; // Exit early
		}
		...
		if (newLocale !== config.fallbackLocale && !translations[newLocale]) {
			if (config.debug) {
				console.log(`Loading translations for locale: ${newLocale}`);
			}
			try {
				// Use dynamic import to load the JSON file
				// const module = await import(`$lib/translations/${newLocale}.json`);
				if (!config.loadTranslation) {
					throw new Error('loadTranslation function not provided');
				}
				//@ts-expect-error - This is a dynamic import, so we can't type it properly
				// but it should be fine since we cannot really pass in other types than currently 
				// defined in the config (Locale and BaseLanguage)
				const module = await config.loadTranslation(newLocale);
				// Store the loaded translations (module.default assumes Vite/Rollup JSON handling)
				console.log(`[Debug] Loaded module structure for ${newLocale}:`, module); // Added for debugging
				// Use type assertion here as well, consistent with the state initialization.
				translations[newLocale] = module as unknown as Paths<BaseLanguage>;
	...
	function t(
		...
		const translationValue = findTranslationValue(
			key,
			localeToUse,
			config.fallbackLocale as Locale,
			translations
		);
		...
		if (translationValue !== undefined) {
			...
			if (
				...
				const pluralForm =
					(translationValue as PluralObject)[count === 1 ? 'one' : 'other'] ??
			...
			// Handle case where value is an object but no count provided (or it's not a PluralObject)
				...
				console.warn(
					`Translation for key '${key}' in locale '${localeToUse}' is an object but used without 'count' for pluralization or is not a simple string.`
				);
				// Fall through
			}
		}
		...
		if (config.debug) {
			...
			console.warn(
				`Translation key not found: '${key}' (locale: ${localeToUse}, fallback attempted: ${localeToUse !== config.fallbackLocale})`
			);
